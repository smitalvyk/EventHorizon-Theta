...
namespace Constructor
{
    public class ShipLayout
    {
            ...
            foreach (var component in predefinedComponents)
            {
                ...
                try
                {
                    ...
                    if (id < 0)
                    {
                        ...
                        // Error log with module name and coordinates
                        Debug.LogWarning($"[ShipBuilder] Error! Failed to install module '{moduleId}' at X:{component.X}, Y:{component.Y}. Check constraints or bounds.");
                        continue;
                    }
        ...
        public IEnumerable<IntegratedComponent> Components => _components.Where(item => item != null);

        public int InstallComponent(ComponentInfo info, int x, int y, int desiredComponentId = -1)
        {
            if (info.Data == null) return -1;
        ...
        {
            var element = _layout[index];
            if (element.ComponentId >= 0) return false;

            // Simplified compatibility check to avoid missing Enum errors
            if (element.Type == CellType.Weapon && component.CellType == CellType.Weapon)
            {
                return string.IsNullOrEmpty(element.WeaponClass);
            }
        ...
        private void UpdateBarrelData(IEnumerable<Barrel> barrels)
        {
            ...
            {
                if (barrel.PositionInLayout < _layout.Length)
                {
        ...
        private bool CheckCell(int x, int y)
        {
            ...
            if (item.Type != CellType.Weapon || item.BarrelId >= 0) return false;

            var changed = false;
            if (x > 0 && TryAssignBarrelId(ref item, _layout[index - 1])) changed = true;
            else if (x + 1 < Size && TryAssignBarrelId(ref item, _layout[index + 1])) changed = true;
            else if (y > 0 && TryAssignBarrelId(ref item, _layout[index - Size])) changed = true;
            else if (y + 1 < Size && TryAssignBarrelId(ref item, _layout[index + Size])) changed = true;

            if (changed) _layout[index] = item;
            return changed;
        }
