...
class CollisionMapGenerator : AssetPostprocessor
{
	public const int CollisionMapSize = 32;
	...
	{
		if (_texturesToDither.Contains(assetPath))
		{
			DitherTexture(texture);
			_texturesToDither.Remove(assetPath);
			return;
		}
    }
	...
	private void DitherTexture(Texture2D texture)
	{
		...
		{
			var texw = texture.width >> i;
			var texh = texture.height >> i;
	...
	private HashSet<string> _texturesToDither = new HashSet<string>();
}
