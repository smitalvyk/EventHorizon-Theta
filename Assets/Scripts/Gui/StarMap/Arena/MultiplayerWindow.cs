...
namespace Gui.Multiplayer
{
    public class MultiplayerWindow : MonoBehaviour
    {
        ...
        private void RefreshAdItem()
        {
            ...
            if (!_adsManager.AdsEnabled) return;

            bool error = false, loading = false, ready = false;
            ...
            _showAdButton.gameObject.SetActive(ready);
            _refreshAdButton.gameObject.SetActive(error);
            _adNotAvailableText.gameObject.SetActive(error);
            _watchAdText.gameObject.SetActive(ready || loading);
        ...
        private bool IsAdButtonVisible
        {
            get
            {
                ...
                if (!_adsManager.AdsEnabled) return false;
                return true;
            }
        }
...
#else
            _timerPanel.SetActive(ready);
            _buyButton.SetActive(status == Status.Ready && availableBattles == 0);
#endif
